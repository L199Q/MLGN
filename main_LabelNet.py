...
from modelLabelNet import MLGN
...
def build_train_model(config):
    ...
    return train_data_loader, test_data_loader, bert, mlgn, tokenizer, classify_loss_function, optimizer, lr_scheduler



def run_train_batch(config, data, bert, mlgn, classify_loss_function, optimizer, lr_scheduler):
...
if __name__ == '__main__':
    ...
    train_data_loader, test_data_loader, bert, mlgn, tokenizer, loss_function, optimizer, lr_scheduler = build_train_model(config)
    ...
    for epoch in range(config['epochs']):
        ...
        with tqdm(train_data_loader, ncols=200) as batch:
            for data in batch:
                text_classify_loss = run_train_batch(config, data, bert, mlgn, loss_function, optimizer, lr_scheduler)
                batch.set_description(f"train epoch:{epoch + 1}/{config['epochs']}")
                batch.set_postfix(text_classify_loss=text_classify_loss.item())
        with torch.no_grad():
            bert.eval()
            mlgn.eval()
            accuracy.reset_acc()
            with tqdm(test_data_loader, ncols=200) as batch:
                for data in batch:
                    ...
                    # batch.set_postfix(loss=loss, p1=p1, p3=p3, p5=p5)
        log_str = f'{epoch:>2}: {p1:.4f}, {p3:.4f}, {p5:.4f}, {d3:.4f}, {d5:.4f}, train_loss:{loss}'
        LOG = Logger(config['bert_version']+'LabelNet')
        LOG.log(log_str)
        if max_only_p5 < p5:
            max_only_p5 = p5
            save(config['bert_version']+'LabelNet', log_str, bert, tokenizer, mlgn)