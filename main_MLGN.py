...
def run_train_batch(config, data, bert, mlgn, classify_loss_function,contrastive_loss, optimizer, lr_scheduler):
    ...
    return text_classify_loss, label_classify_loss, contrastive,loss
...
if __name__ == '__main__':
    ...
    for epoch in range(config['epochs']):
        ...
        with tqdm(train_data_loader, ncols=200) as batch:
            for data in batch:
                text_classify_loss,label_classify_loss,contrastive,loss_train = run_train_batch(config, data, bert, mlgn, loss_function, contrastive_loss, optimizer, lr_scheduler)
        ...
        log_str = f'{epoch:>2}: {p1:.4f}, {p3:.4f}, {p5:.4f}, {d3:.4f}, {d5:.4f}, train_loss:{loss_train.item()}'
        LOG = Logger(config['bert_version']+'MLGN')
        LOG.log(log_str)
        if max_only_p5 < p5:
            max_only_p5 = p5
            save(config['bert_version']+'MLGN', log_str, bert, tokenizer, mlgn)